...
void ERFilterNM::er_tree_extract( InputArray image )
{
    ...
    for (;;) {
		...
		//黑框里面的算法：如果找到局部最小值了，但最小边界值堆里弹出了一个level等同于局部最小值的边界，
		//那这个边界值就要返回到这里的红框循环判断中来。但是巧妙的是：pushu_new_component现在false
		//所以并没有将这个边界值压入极值区域栈！！
        ...
        for ( ; current_edge < 4; current_edge++)
        {
            ...
            if ( !accessible_pixel_mask[neighbour_pixel] && (neighbour_pixel != current_pixel) )
            {
                     ...
                     // to the boundary heap for later processing
                {
					...
					//搭配  for ( ; current_edge < 4; current_edge++) 之外的 if (push_new_component) continue;
					//把当前level放到堆中，然后将新的、level小的像素压入栈中。
		...
		//就在这里！！将上面锁定的局部最小level值得像素点加入极值区域中；
		//也是在这里！！计算递增计算描述子。
		...
		//这可是边界像素点堆弹出的最小灰度值得边界像素点的灰度值。
        int new_level = image_data[current_pixel];

        // if the new pixel has higher grey value than the current one
		//直到说：局部最小值平面确定了，这时候终于从最小值堆里面弹出一个边界值，是大于局部最小值得，才第一次进入处理栈子模块。

		//初始：连最小灰度值的边界像素的灰度值都大于局部最小灰度值像素点的灰度值，那只能说明不存在局部最小灰度值面，而只是一个局部最小点而已，则进入回升阶段；
		//后续：若弹出的最小灰度只边界像素点的灰度值大于该层极值区域面的灰度值，说明这层的极值区域面已经全部探索完毕了，该进入回升阶段
		//从而探索下一个（灰度值增1）的极值区域平面了。
        ...
        if (new_level != current_level) {
			...
			//while里面是处理栈（也就是通过merge来构造ER树的处理过程)的终止条件
			//进行merge后，新的栈顶是和边界值得level一样的，因此

			//一定是检测到边界像素点的灰度值高了，我们才回升！如果一旦回升（也即是极值区域面的灰度值）超过了边界像素的灰度值，
			//那么一定会退出而不让回升！一定是先去填充极值区域面，然后找到更高的边界，才会继续回升！

            while (er_stack.back()->level < new_level)
            {
				//要回升了，首先就是处理栈顶，处理栈顶首先要把它弹出么
...
// Evaluates if a pair of regions is valid or not
// using thresholds learned on training (defined above)
//(abs(grey_mean1-grey_mean2) > PAIR_MAX_INTENSITY_DIST)
//(sqrt(pow(a_mean1-a_mean2,2)+pow(b_mean1-b_mean2,2)) > PAIR_MAX_AB_DIST)
//因此我们需要grey和lab，因此需要原图image
//图割需要channel
bool isValidPair(Mat &grey, Mat &lab, Mat &mask, vector<Mat> &channels, vector< vector<ERStat> >& regions, Vec2i idx1, Vec2i idx2)
{
...
//
////这里为测试aux_region模样，而做的er_draw
//void er_draw(Mat &channels, vector<ERStat> &regions, vector<int> group, Mat& segmentation)
//{
//	for (int r = 0; r<(int)group.size(); r++)
//	{
//		ERStat er = regions[group[r]];
//		//if (er.parent != NULL) // deprecate the root region
//		//{
//			int newMaskVal = 255;
//			int flags = 4 + (newMaskVal << 8) + FLOODFILL_FIXED_RANGE + FLOODFILL_MASK_ONLY;
//			floodFill(channels, segmentation, Point(er.pixel%channels.cols, er.pixel / channels.cols),
//				Scalar(255), 0, Scalar(er.level), Scalar(0), flags);
//			imshow("aux", segmentation);
//			imwrite("aux.jpg", segmentation);
//		//}
//	}
//}
////测试在这里结束
...
//原图image用以isValidPair中的grey、lab的提取；通道src用以图割；regions是两通道、两阶段分类后剩余的极值区域。
    ...
    for(size_t c=0; c<num_channels; c++)
    {
        ...
        for (size_t i=0; i<pending_sequences.size(); i++)
        {
            ...
            for (size_t j=i+1; j<pending_sequences.size(); j++)
            {
                ...
                {
                    expanded = true;
					//pending_sequences是region_sequence类结构体向量，结构体pending_sequences[i]中有成员region_triplet类向量
		...
		//In order to overcome low recall of the v1 function, the text localization is performed twice;
		//In the second run, v1 is forced to 1(也就是不加以neumann多通道、两阶段分类的过滤）and only existing text line hypotheses are taken into account
		//using the hypotheses-verification framework
		...
		if (do_feedback_loop)
		{
			...
			//loadDummyClassifier();创建一个伪分类器（仅仅是实现Callback类而已，没有内容）,用于接受所有区域，即不过滤。
			...
			for (int i = 0; i<(int)valid_sequences.size(); i++)
			{
				...
				////这里是我编的测试代码，想看下src[c](rect）长啥样？
				//if (c == 1)
				//{				
				//char buff[10]; char *buff_ptr = buff;
				//sprintf(buff, "src %d", i);
				//imshow(buff_ptr, src[c](rect));
				//
				//}

				//这里是明天的工作，要改编下er_draw函数，想看下aux_regions长啥样。
				er_filter->run(tmp, aux_regions);

				////测试过程
				//er_filter->run(tmp, aux_regions);
				////这里是编的测试代码，看aux_regions的样子
				//Mat out_aux_region = Mat::zeros(img.rows + 2, img.cols + 2, CV_8UC1);
				//vector<int> aux_region_group;
				//if (c == 1)
				//{
				//	for (int j = 0; j < (int)aux_regions.size(); j++)
				//	{
				//		aux_region_group.push_back(j);
				//	}
				//	//Mat aux_tmp = Mat::zeros(img.rows + 2, img.cols + 2, CV_8UC1);
				//	Mat aux_tmp = Mat::zeros(tmp.rows + 2, tmp.cols + 2, CV_8UC1);
				//	er_draw(tmp, aux_regions, aux_region_group, aux_tmp);
				//	waitKey(0);
				//	/*out_aux_region = out_aux_region | aux_tmp;
				//	imshow("out_aux_region", out_aux_region);*/
				//}
				//	aux_region_group.clear();
				////这里测试代码结束

				//上面的测试是8.13到8.14的工作，下面的工作是弄懂hypothesis verification framework
				...
				for (size_t r = 0; r<aux_regions.size(); r++)
				{
					...
					if (!overlaps)
					{
						...
						//把将设验证框架中没有经过Neumann两阶段分类的ER树的第r个极值区域aux_region[r]压入通道c的经过Neumann两阶段分类后的极值区域向量region[c]
						regions[c].push_back(aux_regions[r]);
						for (size_t j = 0; j<valid_sequences[i].triplets.size(); j++)
						{
							//aux_regions[r]的坐标 Vec2i((int)c, (int)(regions[c].size()) - 1))与valid_sequences[i].triplets[j].a若配对
							//且a在aux_regions[r]右边，则将（距离，a的通道值，a的个数）放入向量right_couples中
			...
			//cout << "hypothesis verification framework done!!" << endl;
		}
        ...
        for (size_t i=0; i<valid_sequences.size(); i++)
        {
            ...
            for (size_t j=0; j<valid_sequences[i].triplets.size(); j++)
            {
				...
				//这里是防止重复的，找不到（== group_regions.end()）就压入向量group_regions
