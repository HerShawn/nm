...
int main(int argc, char* argv[])
{
    ...
    vector< vector<Vec2i> > nm_region_groups,nm_region_groups_before;
    vector<Rect> nm_boxes,nm_boxes_before;
	erGrouping(image, channels, regions, nm_region_groups_before, nm_boxes_before, nm_region_groups, nm_boxes, ERGROUPING_ORIENTATION_HORIZ);
	...
	Mat out_img_segmentation_before = Mat::zeros(image.rows + 2, image.cols + 2, CV_8UC1);
    ...
    for (int i=0; i<(int)nm_boxes.size(); i++)
    {
		...
		Mat group_img_before = Mat::zeros(image.rows + 2, image.cols + 2, CV_8UC1);
		//vector< vector<Vec2i> >& nm_region_groups
		//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
		//er_draw(channels, regions, nm_region_groups_before[i], group_img_before);
        er_draw(channels, regions, nm_region_groups[i], group_img);
		//Mat group_segmentation_before;
        Mat group_segmentation;
		//group_img_before.copyTo(group_segmentation_before);
        ...
        for (int j=0; j<(int)boxes.size(); j++)
        {
			...
			//out_img_segmentation_before = out_img_segmentation_before | group_segmentation_before;
        }
	...
	//imshow("文字行合并后图割结果_before（ergrouping的nm_region_groups经floodfill图割得出）", out_img_segmentation_before);
	//imwrite("segmentation_before.jpg", out_img_segmentation_before);
